...
RUN groupadd -g 1000 euroCar && \
	useradd -ms /bin/bash -u 1000 -g 1000 euroCar && \
	mkdir -p /app && \
	chown euroCar:euroCar /app
...
COPY --chown=euroCar:euroCar package.json .
COPY --chown=euroCar:euroCar yarn.lock .

RUN yarn install --frozen-lockfile && yarn cache clean

COPY --chown=euroCar:euroCar src src/
